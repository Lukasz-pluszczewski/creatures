...
export const NeuronVisualization = ({ creature, neurons, config, isLoading }) => {
  useEffect(() => {
    ...
    if (creature && !isLoading) {
      ...
      const edges = new vis.DataSet(creature.genome.map(gene => {
        const [, sourceId, , targetId, rawWeight] = gene;
  ...
  }, [creature, neurons, config, isLoading]);
